...
The governing rule: **complex values are never rendered inline** — structure is browsed one table at a time, and anything deeper is reached by drilling, not by scrolling. Nested dict/list *values* split by depth (`_cell()` + `_is_deep()`): **flat** values (scalars only — a channels map, an RGB color) are rendered as indented **YAML** blocks (via `yaml.safe_dump`) — YAML's block style has no braces/quotes/commas to fight through, so it stays readable even wrapped mid-structure, which made it a better fit here than JSON once `pyyaml` was already a dependency for `-y/--yaml`. **Deep** values (a dict/list that itself contains dicts/lists) render two ways, neither inline: in *table cells* they collapse to a count (`{...} (N fields)` / `[...] (1 field)` — pluralized); in a *kv view* (a get's mixed payload) they get a `see '<field>' table below` marker and render as their own titled table after the kv block, via `_render_value(value, title=..., defer=False)` — `defer=False` means that sub-table level summarizes *its* deep values to counts instead of deferring again, so output is bounded at exactly two levels (kv → sub-tables) no matter how deep the payload nests; a deferred mixed dict becomes a kv sub-table with counts, and a deferred irregular list (not all-dicts) renders one summarized cell per item, never a full YAML dump. Going deeper than one sub-table uses the get commands' **PATH drill-down** (see the `_shared.py` paragraph below) or `-j`/`-y`. Rows grow taller instead of columns growing wider: `print_table()`/`print_kv()` set `show_lines=True` (so multi-line cells stay visually separated per row) and `overflow="fold"` per column (fold-wrap instead of Rich's default ellipsis-truncate). `print_table()` additionally caps every column's `max_width` (`_SCALAR_COLUMN_MAX` / `_NESTED_COLUMN_MAX`) so one unbroken 40-char serial/id string can't dominate Rich's shrink-widest-column-first pass and starve a YAML column down to nothing (see "Verified against real hardware" for what changed here and why).
