...
The governing rule: **complex values are never rendered inline** — structure is browsed one table at a time, and anything deeper is reached by drilling, not by scrolling. Nested dict/list *values* split by depth (`_cell()` + `_is_deep()`): **flat** values (scalars only — a channels map, an RGB color) are rendered as indented **YAML** blocks (via `yaml.safe_dump`) — YAML's block style has no braces/quotes/commas to fight through, so it stays readable even wrapped mid-structure, which made it a better fit here than JSON once `pyyaml` was already a dependency for `-y/--yaml`. **Deep** values (a dict/list that itself contains dicts/lists) render two ways, neither inline: in *table cells* they collapse to a count (`{...} (N fields)` / `[...] (N items)`); in a *kv view* (a get's mixed payload) they get a `see '<field>' table below` marker and render as their own titled table after the kv block (`_render_subtable()` — dict-of-dicts/list-of-dicts become rows; a mixed dict recurses into another kv+sub-tables level). Going deeper than one sub-table uses the get commands' **PATH drill-down** (see the `_shared.py` paragraph below) or `-j`/`-y`. Rows grow taller instead of columns growing wider: `print_table()`/`print_kv()` set `show_lines=True` (so multi-line cells stay visually separated per row) and `overflow="fold"` per column (fold-wrap instead of Rich's default ellipsis-truncate). `print_table()` additionally caps every column's `max_width` (`_SCALAR_COLUMN_MAX` / `_NESTED_COLUMN_MAX`) so one unbroken 40-char serial/id string can't dominate Rich's shrink-widest-column-first pass and starve a YAML column down to nothing (see "Verified against real hardware" for what changed here and why).
...
**`commands/_shared.py`'s `drill()`/`render_subtree()`** give every single-resource get (`devices get`, `led get`, `color get`, `temperatures get`, `macro get`) a variadic **PATH** argument that walks into the payload one segment at a time — dict keys matched case-insensitively (the API mixes casings and uses numeric-string keys), list items by index — e.g. `olh led get <id> devices 1 channels` for one channel's per-LED color table, or `olh devices get <id> userProfiles`. The subtree renders through the normal shape router, and `-j`/`-y` emit just that subtree, jq-ready; a bad segment raises `click.UsageError` naming where it failed and the available fields. No per-key *write* subcommands were added deliberately: the upstream API has no endpoint for setting arbitrary payload fields — every writable setting already has its dedicated endpoint wrapped by the existing groups (speed/color/brightness/label/position/profiles/...).
