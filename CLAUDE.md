...
**`output.py`** is the one place response-shape decisions get made, so individual commands stay one-liners: `obj.render(response, key="data")`. `render()` extracts `response[key]` (when present) and routes by shape — dict-of-dicts → table (each sub-dict's key injected as an `id` column), list-of-dicts → table, single dict → a two-column key/value table (*flat* nested dicts flattened one level as `field.subfield` rows, an *empty* dict as a literal `{}` row rather than silently vanishing, and *deep* fields deferred to sub-tables — see the never-inline rule below), scalar → printed directly, empty dict/list → `"No data returned."` rather than silent blank output (see "Verified against real hardware" — this one was a real bug, not a hypothetical). All rendered text — cell values, field names, column headers, titles, ack messages — is passed through `rich.markup.escape()` at the render boundary: API data and user-assigned labels can contain markup-shaped text (`[/x]`), which would otherwise crash Rich with `MarkupError` (an uncaught non-`OpenLinkHubError`, i.e. a raw traceback) or silently restyle output. `-j/--json`/`-y/--yaml` (global flags, resolved to `CliContext.output_format`, one of `"table"`/`"json"`/`"yaml"`) bypass all of that and pretty-print the raw envelope in the requested format.

The governing rule: **complex values are never rendered inline** — structure is browsed one table at a time, and anything deeper is reached by drilling, not by scrolling. Nested dict/list *values* split by depth (`_cell()` + `_is_deep()`): **flat** values (scalars only — a channels map, an RGB color) are rendered as indented **YAML** blocks (via `yaml.safe_dump`) — YAML's block style has no braces/quotes/commas to fight through, so it stays readable even wrapped mid-structure, which made it a better fit here than JSON once `pyyaml` was already a dependency for `-y/--yaml`. **Deep** values (a dict/list that itself contains dicts/lists) render two ways, neither inline: in *table cells* they collapse to a count (`{...} (N fields)` / `[...] (1 field)` — pluralized); in a *kv view* (a get's mixed payload) they collapse **by default** to that same count plus a dim hint (`add '<field>' to the command, or --all`) — each table is shown only when asked for, either by naming it as a PATH segment (`devices get <id> DeviceProfile`) or by passing the get commands' shared `--all` flag (`all_option` in `commands/_shared.py`, threaded through `render()`/`print_kv()` as `expand`). With `--all`, deep fields get a `see '<field>' table below` marker and render as their own titled table after the kv block, via `_render_value(value, title=..., defer=False)` — `defer=False` means that sub-table level summarizes *its* deep values to counts instead of deferring again, so even `--all` output is bounded at exactly two levels (kv → sub-tables) no matter how deep the payload nests; a deferred mixed dict becomes a kv sub-table with counts, and a deferred irregular list (not all-dicts) renders one summarized cell per item, never a full YAML dump. Going deeper than one sub-table uses the get commands' **PATH drill-down** (see the `_shared.py` paragraph below) or `-j`/`-y`; `--all` composes with a PATH, expanding one level below wherever the PATH landed. Rows grow taller instead of columns growing wider: `print_table()`/`print_kv()` set `show_lines=True` (so multi-line cells stay visually separated per row) and `overflow="fold"` per column (fold-wrap instead of Rich's default ellipsis-truncate). `print_table()` additionally caps every column's `max_width` (`_SCALAR_COLUMN_MAX` / `_NESTED_COLUMN_MAX`) so one unbroken 40-char serial/id string can't dominate Rich's shrink-widest-column-first pass and starve a YAML column down to nothing (see "Verified against real hardware" for what changed here and why).

`devices list` (in `commands/devices.py`) goes further than the generic deep-cell summarization — it's the one exception to "commands stay one-liners": it synthesizes a `channels` column — `{channelId: "label (description)"}` per hub, inserted just before the `GetDevice` column — and reduces the `GetDevice` cell to its **string-valued keys only** (manufacturer/product/serial/firmware/...). Table mode only; `-j`/`-y` on `devices list` still emit the untouched raw envelope.

**`commands/_shared.py`'s `drill()`/`render_subtree()`/`path_argument`** give every drill-capable get (`devices all`, `devices get`, `led get`, `color get`, `temperatures get`, `temperatures graph`, `macro get`) a variadic **PATH** argument that walks into the payload one segment at a time — dict keys by exact match first, then case-insensitively (the API mixes casings and uses numeric-string keys) with an ambiguity error if several keys collide under casefold, list items by index — e.g. `olh led get <id> devices 1 channels` for one channel's per-LED color table, or `olh devices get <id> userProfiles`. The subtree renders through the normal shape router, and `-j`/`-y` emit just that subtree, jq-ready; a bad segment raises `click.UsageError` naming where it failed and the available fields. `render_subtree` mirrors `render()`'s envelope handling (payload under `key` when present, else the whole response), so anything visible without a PATH is also drillable with one. No per-key *write* subcommands were added deliberately: the upstream API has no endpoint for setting arbitrary payload fields — every writable setting already has its dedicated endpoint wrapped by the existing groups (speed/color/brightness/label/position/profiles/...).
...
1. **Table rendering under real terminal width.** `devices all`/`devices list` initially dumped nested objects (`GetDevice`, `defaultColor`, `profiles`, ...) as inline JSON in table cells. Against the real API's actual response shape (a `GetDevice` payload with 36+ fields, itself containing a `devices` map of 20+ sub-devices each with 40 fields), this wrapped into an unreadable multi-thousand-character mess. First fixed by summarizing nested cell values (`{...} (36 fields, use -j/--json for detail)`) instead of inlining them; revisited (2026-07-18) to show full nested detail as indented YAML blocks; then settled (2026-07-19) on "complex values are never inline" as described in the `output.py` paragraph above — flat values as YAML, deep values as a count in table cells and as titled sub-tables in kv views, with deeper structure reached via the gets' PATH drill-down — because full-YAML rendering was only readable for `devices list` after its bespoke string-filtering, while every other group's list *and get* (`led`, `color`, `temperatures`, ...) still drowned in nested blobs (a real hub's `led get` payload is 12 channels × 44 LEDs × 7 color fields).
...
- `test_output.py` — `render()`'s shape-routing (table/kv/scalar/empty), `-j/--json` and `-y/--yaml` bypass, the never-inline rules (deep table cells summarized to a count; flat cells rendered in full; kv views collapsing deep fields to a count-plus-hint by default and, under `expand`/`--all`, deferring them to titled sub-tables that are bounded — a deferred mixed dict summarizes to counts instead of recursing, a deferred irregular list never falls back to an inline YAML dump), empty-dict fields rendering as `{}` rows instead of vanishing, markup-shaped text in labels/field names rendering literally instead of crashing Rich, `confirm_or_abort()`'s three paths (`yes=True` skip, declined, accepted).
- `test_drill.py` — pure-unit coverage of `drill()`: multi-level walking, exact-match-beats-casefold, ambiguous casefold collision, unknown key / bad index / scalar node errors naming where they failed.
- `test_cli_devices.py` — representative GET-collection and GET-single, the default hide-sub-tables/`--all` behavior end-to-end, plus the global `-j`/`-y` flags end-to-end through a real command, and that passing both together is a `click.UsageError`.
