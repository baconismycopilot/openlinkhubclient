...
One exception to "commands stay one-liners": `devices list` (in `commands/devices.py`) synthesizes a `channels` column — `{channelId: "label (description)"}` per hub, inserted just before the `GetDevice` column — and reduces the `GetDevice` cell to its **string-valued keys only** (manufacturer/product/serial/firmware/...). The structured settings that filter hides (`devices` map, `userProfiles`, flags, brightness, ...) are viewed by drilling in with `olh devices get <id> [KEY]` (KEY matched case-insensitively; the subtree renders through the normal shape router, and `-j`/`-y` emit just that subtree, jq-ready). No per-key *write* subcommands were added deliberately: the upstream API has no endpoint for setting arbitrary `GetDevice` fields — every writable setting already has its dedicated endpoint wrapped by the existing groups (speed/color/brightness/label/position/profiles/...). Table mode only; `-j`/`-y` on `devices list` still emit the untouched raw envelope.
